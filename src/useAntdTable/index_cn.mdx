...
import DemoTabs from './demo/demo_tabs/index';
import DemoTabsServiceCodeTsx from '!raw-loader!./demo/demo_tabs/service.ts';
import DemoTabsListCodeTsx from '!raw-loader!./demo/demo_tabs/List.tsx';
import DemoTabsTodosCodeTsx from '!raw-loader!./demo/demo_tabs/Todos.tsx';
import DemoTabsIndexCodeTsx from '!raw-loader!./demo/demo_tabs/index.tsx';
...
* 支持数据缓存，浏览器刷新、从其他页面切换到该页面、组件销毁重建时能自动还原上一次的表单数据，分页状态等
...
<JackBox tsCode={
  `
// ---------------------------------------------------- service.ts Start -------------------------------------------------------
${DemoTabsServiceCodeTsx}

// ---------------------------------------------------- List.tsx Start -------------------------------------------------------
${DemoTabsListCodeTsx}

// ---------------------------------------------------- Todos.tsx Start -------------------------------------------------------
${DemoTabsTodosCodeTsx}

// ---------------------------------------------------- index.tsx Start -------------------------------------------------------
${DemoTabsIndexCodeTsx}
`
} demoName='数据缓存-tabs' description='如果 options 传了 id 参数，则我们会将当前表单数据、分页数据、筛选排序等数据全部缓存下来，刷新进入或者从其他页面返回时调用activeCache后再路由到该页面将会使用记录的搜索条件来搜索，如果在同一个页面切换tab页面，则会保留搜索条件(重置页码、排序、页容等)'>
  <DemoTabs />
</JackBox>
...
| id              | 表格唯一 id，如果有 id，则会自动缓存并恢复数据,建议使用pathname(根据情况再带上页面唯一key)                               | string | -      |
| form            | antd form 实例，如果有 form 实例，则 asyncFn 会收到 form 数据                | -      | -      |
| formatResult    | 对 asyncFn 返回数据进行格式化，如果 asyncFn 返回数据符合要求，则不需要该参数 | -      | -      |

## 还原搜索条件、页码等

从其他页面跳转到本页面，如果需要还原之前的搜索状态

```
 import {activeCache} from '@wetrial/useAntdTable';
  // 将缓存标记为激活状态
  activeCache('/sales/order')
  // 跳转到该页面
